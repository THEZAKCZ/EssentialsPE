...
import EssentialsPE.Events.PlayerAFKModeChangeEvent;
import EssentialsPE.Events.PlayerFlyModeChangeEvent;
import EssentialsPE.Events.PlayerGodModeChangeEvent;
import EssentialsPE.Events.PlayerMuteEvent;
import EssentialsPE.Events.PlayerNickChangeEvent;
import EssentialsPE.Events.PlayerPvPModeChangeEvent;
import EssentialsPE.Events.PlayerUnlimitedModeChangeEvent;
import EssentialsPE.Events.PlayerVanishEvent;
import EssentialsPE.Events.SessionCreateEvent;
import EssentialsPE.Loader;
import EssentialsPE.Tasks.AFK.AFKKickTask;
import EssentialsPE.Tasks.AFK.AFKSetterTask;
import EssentialsPE.Tasks.GeoLocation;
import EssentialsPE.Tasks.TPRequestTask;
import EssentialsPE.Tasks.Updater.AutoFetchCallerTask;
import EssentialsPE.Tasks.Updater.UpdateFetchTask;
import EssentialsPE.Tasks.Updater.UpdateInstallTask;
...
public class BaseAPI{
    ...
    /** @var array */
    private HashMap<String, BaseLocation> warps = new HashMap<>();

    public BaseAPI(Loader ess){
        this.ess = ess;
        instance = this;
        saveConfigs();
    ...
    public final Loader getEssentialsPEPlugin(){
        return ess;
    }

    public Server getServer(){
        return getEssentialsPEPlugin().getServer();
    }
    ...
    private final void saveConfigs(){
        loadKits();
        loadWarps();
    }

    private final void loadKits(){
        Config cfg = new Config(getEssentialsPEPlugin().getDataFolder() + "Kits.yml", Config.YAML);
        Map<String, Boolean> children = new HashMap<>();

        for(Map.Entry entry : cfg.getAll().entrySet()){
            String key = (String) entry.getKey();
            Object[] value = (Object[]) entry.getValue();

            this.kits.put(key, new BaseKit(key, value));
            children.put("essentials.kits." + key, false);
        }

        getServer().getPluginManager().addPermission(new Permission("essentials.kits", null, null, children));
    }

    private final void loadWarps(){
        Config cfg = new Config(getEssentialsPEPlugin().getDataFolder() + "Warps.yml", Config.YAML);
        Map<String, Boolean> children = new HashMap<>();
        ...
        for(Map.Entry entry : cfg.getAll().entrySet()){
            ...
            String levelName = (String) value[3];

            if(getServer().isLevelGenerated(levelName)){
                if(!this.getServer().isLevelLoaded(levelName)){
                    this.getServer().loadLevel(levelName);
                }
                this.warps.put(key, new BaseLocation(key, (double) value[0], (double) value[1], (double) value[2], getServer().getLevelByName(levelName), (double) value[4], (double) value[5]));
                children.put("essentials.warps." + key, false);
            }
        }
        getServer().getPluginManager().addPermission(new Permission("essentials.warps", null, null, children));
    }

    private final void encodeWarps(){
        encodeWarps(false);
    }

    private final void encodeWarps(boolean save){
        LinkedHashMap<String, Object> warps = new LinkedHashMap<>();
        LinkedHashMap<String, BaseLocation> warps2 = new LinkedHashMap<>();

        for(Map.Entry entry : this.warps.entrySet()){
            String name = (String) entry.getKey();
            Object object = entry.getValue();

            if(object instanceof BaseLocation){
                BaseLocation loc = (BaseLocation) object;

                warps2.put(name, loc);
                warps.put(name, new Object[]{loc.getX(), loc.getY(), loc.getZ(), loc.getLevel().getName(), loc.getYaw(), loc.getPitch()});
            }
        }
        if(save && warps.size() > 0){
            Config cfg = new Config(getEssentialsPEPlugin().getDataFolder() + "Warps.yml", Config.YAML);
            cfg.setAll(warps);
            cfg.save();
        }
    ...
    public void reloadFiles(){
        getEssentialsPEPlugin().getConfig().reload();
        //$this->economy->reload();
        loadKits();
        loadWarps();
    }
    ...
    public static final ArrayList<Integer> NON_SOLID_BLOCKS = new ArrayList<>(Arrays.asList(Block.SAPLING, Block.WATER, Block.STILL_WATER, Block.LAVA, Block.STILL_LAVA, Block.COBWEB, Block.TALL_GRASS, Block.BUSH, Block.DANDELION,
    ...
    public boolean isAFK(Player player){
        return getSession(player).isAFK();
    }
    ...
    public boolean setAFKMode(Player player, boolean state){
        return setAFKMode(player, state, true);
    }

    public boolean setAFKMode(Player player, boolean state, boolean broadcast){
        PlayerAFKModeChangeEvent ev = new PlayerAFKModeChangeEvent(this, player, state, broadcast);
